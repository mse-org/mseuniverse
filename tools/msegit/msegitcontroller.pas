 ...
 if not result and not fversionchecked then begin
  ...
  {$ifdef mswindows}
   ...
   if {findfile(mstr1) and} not getconfig('core.askpass',mstr2) then begin
    mstr1:= encodepathparam(mstr1,true);
    fcommandlinevars:= fcommandlinevars+' -c core.askpass=' + mstr1;
 ...
 //    'show --format=format:"%x01%H%x01%cN%x01%ct%x01%s%x01" -s '+str1,mstr1);
   ...
   if mstr1 <> '' then begin
    ...
    while int1 < int2 do begin
     ...
     inc(int1);
     while (int1 <= high(ar1)) and (ar1[int1] = '') do begin  //copy duplicates
