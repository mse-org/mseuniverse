...
type
 tmainfo = class(tmseform)
   ...
   tbutton24: tbutton;
   paper: tstringgrid;
   mainLayouter: tlayouter;
   buttonLayouter: tlayouter;
...
begin
 ...
 if error then begin
  ...
  paper.appendrow;
  paper[1][paper.rowcount-1] := '-----------------------------------';
  s := FloatToStrf(total,ffNumber,maxdigits,decimals);
  //fmtstr(s, '%s %22s', ['=', s]);
  paper.appendrow('=');
  paper[1][paper.rowcount-1] := s;
  paper.appendrow;
...
begin
  ldisplay.caption := errormsg;
  paper.appendrow;
  paper[1][paper.rowcount-1] := errormsg;
  paper.appendrow;
  paper.frame.sbvert.value:= 1;
end;
