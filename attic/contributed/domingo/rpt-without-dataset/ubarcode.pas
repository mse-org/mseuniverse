...
{ 
  MSE port by Attid (Tolstov Igor)
  27.03.2007
  attid@yandex.ru
...
uses
  Classes, msegraphics, msegraphutils, msedrawtext;
...
//    constructor Create(Owner:TComponent); override;
    ...
    procedure DrawTexts(Canvas:TCanvas);
  ...
  published
   ...
   { 0 - 360 degree }
    ...
    property Color:Colorty read FColor write FColor default cl_White;
    property ColorBar:Colorty read FColorBar write FColorBar default cl_Black;
...
function TranslateQuad2D(const alpha :double; const orgin, point :pointty): pointty;
var
   alphacos: Extended;
   alphasin: Extended;
   moveby:   pointty;
...
{Zuordung der Paraitaetsfolgen fьr EAN13}
...
begin
  ...
  begin
    ...
    {Falls i ungerade ist dann mache Lьcke zu Strich}
      if odd(j) then
        c := chr(ord(c)+5);
      result := result + c;
    end;
   result := result + '0';   {Lьcke zwischen den Zeichen}
  end;
...
begin
  ...
  if FCheckSum then
    begin
    checksuml := checksuml mod 43;
    for i:=0 to High(tabelle_39) do
      if checksuml = tabelle_39[i].chk then
      begin
...
begin
  case FTyp of
    bcCode128A, bcCodeEAN128A:
      begin checksuml := 103; startcode:= StartA; end;
    bcCode128B, bcCodeEAN128B:
      begin checksuml := 104; startcode:= StartB; end;
    bcCode128C, bcCodeEAN128C:
      begin checksuml := 105; startcode:= StartC; end;
    else
  ...
  checksuml := checksuml mod 103;
  result := result + tabelle_128[checksuml].data;
...
var
  i:integer;
  check_even, check_odd, checksuml:integer;
begin
  result := '60';    {Startcode}
  check_even := 0;
  check_odd  := 0;
  ...
  checksuml := quersumme(check_odd*2) + check_even;

  checksuml := checksuml mod 10;
  if checksuml > 0 then
    checksuml := 10-checksuml;

  result := result + tabelle_MSI[chr(ord('0')+checksuml)];
...
begin
 ...
 // orgin := TranslateQuad2D(alpha,orgin,makepoint(Width,Height));// not port
  ...
  with Canvas do begin
   ...
   for i:=1 to Length(data) do  {examine the pattern string}
    begin
      ...
      OneBarProps(data[i], widthline, lt);
...
//?????      Brush.Color := Pen.Color;  // not port

      if lt = black_half then
        heightline := FHeight * 2 div 5
      else
        heightline := FHeight;
      ...
      c.x := xadd+Widthline-1;  {23.04.1999 Line was 1 Pixel too wide}
      c.y := Height;

      d.x := xadd+Widthline-1;  {23.04.1999 Line was 1 Pixel too wide}
      d.y := 0;
    ...
    {draw the rectangle}
      fillpolygon([a,b,c,d],Pencolor); //overload;
//      fillpolygon([a,b,c,d],color,cl_none);
//      fillpolygon([a,b,c,d]);
...
begin
  ...
  try
    ...
    if FShowText <> bcoNone then
      DrawTexts(Canvas);   {show readable Text}
...
procedure TAsBarcode.DrawTexts(Canvas:TCanvas);
var
  PosX, PosY: Integer;
  rect:rectty;
  wfont: tfont;
...
//      Font.Assign(ShowTextFont);
      try
//        Pencolor := Font.Color;
        wfont.colorbackground := cl_White;
...
// As stated by Andreas Schmidt
        ...
        if ShowTextPosition in [stpTopLeft, stpBottomLeft] then
          ...
          if ShowTextPosition in [stpTopRight, stpBottomRight] then//twidget.getcanvas.getstringwidth() 
            PosX := FLeft + Width - (self.width div 2) //????TextWidth(Text) // not port
          else
            // i know, the last IF is useless, but i like it this way
            if ShowTextPosition in [stpTopCenter, stpBottomCenter] then
              PosX := FLeft + (self.width div 2);//Trunc((Width - TextWidth(Text))/2); // not port
        ...
        if ShowTextPosition in [stpTopLeft, stpTopCenter, stpTopRight] then
          ...
          if ShowTextPosition in [stpBottomLeft, stpBottomCenter, stpBottomRight] then
            PosY := FTop + Height - (self.width div 2);//???? TextHeight(Text); // not port

        if FShowText in [bcoCode, bcoBoth] then
        begin
          rect.x:=PosX;
          rect.y:=PosY;
          drawtext(canvas, FText, rect,[],wfont,nil); {contents of Barcode}
        end;

        if FShowText in [bcoTyp, bcoBoth] then
        begin
          rect.x:=FLeft;
          rect.y:=FTop+Round(Font.Height*2.5);
          drawtext(canvas, GetTypText, rect,[],wfont,nil); {type/name of barcode}
        end;
